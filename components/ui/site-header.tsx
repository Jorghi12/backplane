  ...
  if (!user) {
    return (
      <div className="flex items-center gap-3">
        ...
        <Button asChild className="rounded-full"><Link href="/sign-up">Sign up</Link></Button>
      </div>
    );
  }

  const initials =
    (user.name?.trim().split(/\s+/).map(n => n[0]?.toUpperCase()).slice(0,2).join('')) ||
    (user.email ? user.email.slice(0,2).toUpperCase() : 'U');

  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button aria-label="Open user menu" className="rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">
...
export default function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:inset-x-0 focus:top-0 focus:z-50 focus:m-2 focus:rounded-md focus:bg-white focus:px-3 focus:py-2 focus:shadow">
        Skip to content
      </a>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center">
          ...
          <span className="ml-2 text-xl font-semibold text-gray-900">TrustPlane</span>
        </Link>
        ...
        <div className="md:hidden flex items-center gap-3">
          ...
          <button
            aria-label="Toggle menu" aria-expanded={mobileOpen} aria-controls={navId}
      ...
      <div
        id={navId}
        className={['md:hidden border-t border-gray-200 bg-white transition-[max-height] overflow-hidden', mobileOpen ? 'max-h-96' : 'max-h-0'].join(' ')}
      >
