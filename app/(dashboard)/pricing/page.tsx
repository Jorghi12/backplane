...
export default async function PricingPage() {
  ...
  const findProductId = (name: string) =>
    (products as Product[]).find((p) => p.name === name)?.id;

  const findPrice = (productName: string, interval: 'month' | 'year') => {
    const pid = findProductId(productName);
    return (prices as Price[]).find(
      (p) => p.productId === pid && p.interval === interval
    );
  };
  ...
  const baseTrial =
    baseMonthly?.trialPeriodDays ?? baseYearly?.trialPeriodDays ?? 7;

  // Plus
  const plusMonthly = findPrice('Plus', 'month');
  const plusYearly = findPrice('Plus', 'year');
  const plusTrial =
    plusMonthly?.trialPeriodDays ?? plusYearly?.trialPeriodDays ?? 7;
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <section className="text-center mb-12">
        ...
        <h1 className="mt-4 text-4xl md:text-5xl font-semibold tracking-tight text-gray-900">
          Pricing that scales beyond pilots
        </h1>
        <p className="mt-3 text-gray-600 max-w-2xl mx-auto">
          Start with a {Math.max(baseTrial, plusTrial)}‑day free trial. Bring your own clouds
          and models—no compute resell.
        </p>
      </section>
...
function savePercent(monthlyCents?: number, yearlyCents?: number) {
  ...
  return Math.max(
    0,
    Math.round((1 - yearlyCents / annualAtMonthly) * 100)
  );
}
...
function PlanCard({
  ...
  return (
    ...
    >
      {badge ? (
        <div className="absolute -top-3 left-6 inline-flex items-center rounded-full bg-orange-600 px-3 py-1 text-xs font-semibold text-white shadow">
          {badge}
        </div>
      ) : null}
      ...
      <div className="mt-4">
        <div className="text-4xl font-semibold text-gray-900">
          {formatUsd(monthly)}{' '}
          <span className="text-base font-normal text-gray-600">{intervalLabel}</span>
        </div>
        {yearly ? (
      ...
      <div className="mt-6 space-y-3">
        ...
        {yearlyPriceId ? (
          <form action={checkoutAction}>
            ...
            <SubmitButton
              label={typeof savings === 'number' ? `Start annual — save ${savings}%` : 'Start annual'}
    ...
    <div className="relative rounded-2xl border border-gray-200 bg-white/60 p-6 shadow-sm">
      ...
      <div className="mt-6">
        <Link
          ...
          className="inline-flex w-full items-center justify-center rounded-full bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800"
        >
          Contact sales
        </Link>
      </div>
      ...
      <div className="mt-4 grid gap-4 rounded-xl border border-gray-200 bg-white/60 p-6 md:grid-cols-3">
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Unlimited usage</li>
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Unlimited workspace members</li>
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Email support</li>
          </ul>
        </div>
        <div>
          <h4 className="font-medium text-gray-900">Plus</h4>
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Everything in Base</li>
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Early access to new features</li>
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />24/7 support + Slack access</li>
          </ul>
        </div>
        <div>
          <h4 className="font-medium text-gray-900">Enterprise</h4>
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />In‑VPC deployment & multi‑region</li>
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Advanced governance & audit</li>
            <li className="flex"><Check className="mr-2 h-5 w-5 text-orange-500" />Priority on‑call & dedicated Slack</li>
          </ul>
        </div>
      </div>
    </section>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <details className="rounded-lg border border-gray-200 bg-white/60 p-4">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Pricing is per user, with optional annual billing. Usage‑based or %‑of‑model‑spend
            fees can be added for enterprise contracts when you route inference through the
            control plane.
          </p>
        </details>
        ...
        <details className="rounded-lg border border-gray-200 bg-white/60 p-4">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Yes—deployment is in your cloud account so data and compute stay within your perimeter.
          </p>
        </details>
        ...
        <details className="rounded-lg border border-gray-200 bg-white/60 p-4">
          ...
          <p className="mt-2 text-sm text-gray-700">
            You can convert to monthly or annual at any time. If you don’t subscribe, your
            environment is deactivated without data leaving your account.
          </p>
        </details>
        ...
        <details className="rounded-lg border border-gray-200 bg-white/60 p-4">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Yes—Enterprise can transact via AWS/GCP/Azure marketplaces to streamline procurement.
          </p>
        </details>
      </div>
    </section>
